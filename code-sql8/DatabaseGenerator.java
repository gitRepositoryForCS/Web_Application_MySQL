...
import java.util.*;
 ...
 /**
	Generates a synthetic database for use in CIS 560 SQL6 (MRHC database).
...
public class DatabaseGenerator {
	...
	public static void main (String[] args) throws Exception {
		...
		dbgen.writePatientTable("tablePatient.sql");
		dbgen.writeDiseaseTable("tableDisease.sql");
		dbgen.writeDoctorTable("tableDoctor.sql");
		dbgen.writeSeesTable("tableSees.sql");
		dbgen.writeProductTable("tableProduct.sql");
		dbgen.writeStockTable("tableStock.sql");
		dbgen.writeSupplierTable("tableSupplier.sql");
		dbgen.writeSuppliesTable("tableSupplies.sql");
	}
	...
	public void writePatientTable(String filename) throws FileNotFoundException {
		...
		try {
			...
			String header = "INSERT INTO `Patient` (`pid`, `fname`, `lname`, `age`, `street`, `city`, `zipcode`) VALUES";
			outFile.println(header);
			for (int ipatient = 1; ipatient < getCountPatients(); ipatient++) {
				outFile.println("(" + makePatientRecord(ipatient)+"),");
			}
			outFile.println("(" + makePatientRecord(getCountPatients())+");");
		} finally {
	...
	public void writeDiseaseTable(String filename) throws Exception {
		...
		try {
			outFile = new PrintWriter (filename);
			String header = "INSERT INTO `Disease` (`pid`, `disease`) VALUES";
			outFile.println(header);

			//generate [getTotalDiseases()-1] records (each ends with ",")
	...
	public void writeDoctorTable(String filename) throws FileNotFoundException {
		...
		try {
			outFile = new PrintWriter (filename);
			String header = "INSERT INTO `Doctor` (`did`, `fname`, `lname`, `specialty`) VALUES";
			outFile.println(header);

			//first [getCountDoctors -1] records (each records ends with ",")
			for (int idoctor = 1; idoctor < getCountDoctors(); idoctor++) {
				outFile.println("(" + makeDoctorRecord(idoctor) + "),");	
			}
			//last record (ends with ";")
			outFile.println("(" + makeDoctorRecord(getCountDoctors())  + ");");
		} finally {
			if (outFile != null) outFile.close();
		}
	}
	...
	public void writeSeesTable(String filename) throws Exception {
		...
		try {
			outFile = new PrintWriter (filename);
			String header = "INSERT INTO `Sees` (`pid`, `did`) VALUES";
			outFile.println(header);
			
			//generate first [getTotalSees()-1] records (each ends with ",") 
			...
			while (flag==0)
			{
			//generate last record (ends with ";")
			int patientId = 1 + rand.nextInt(getCountPatients());
			int doctorId = 1 + rand.nextInt(getCountDoctors());
	...
	public void writeProductTable(String filename) throws FileNotFoundException {
		...
		try {
			outFile = new PrintWriter (filename);
			String header = "INSERT INTO `Product` (`eid`, `description`) VALUES";
			outFile.println(header);
			
			for (int iproduct = 0; iproduct < PRODUCT_DESCRIPTIONS.length-1; iproduct++) {
				outFile.println("(" + (iproduct+1) + FIELD_SEP + "'" + PRODUCT_DESCRIPTIONS[iproduct] + "'" + "),");
			}
			outFile.println("(" + PRODUCT_DESCRIPTIONS.length + FIELD_SEP + "'" + PRODUCT_DESCRIPTIONS[PRODUCT_DESCRIPTIONS.length-1] + "'" + ");");	
	...
	public void writeStockTable(String filename) throws FileNotFoundException {
		...
		String header = "INSERT INTO `Stock` (`eid`, `quantity`) VALUES";
		outFile.println(header);
		...
		try {
			...
			outFile.println("(" + PRODUCT_DESCRIPTIONS.length + FIELD_SEP + (1+rand.nextInt(getMaxStock())) + ");");
		} finally {
			if (outFile != null) outFile.close();
		}
	}
	...
	public void writeSupplierTable(String filename) throws FileNotFoundException {
		PrintWriter outFile = new PrintWriter (filename);
		String header = "INSERT INTO `Supplier` (`sid`, `name`, `street`, `city`, `zipcode`) VALUES";
		outFile.println(header);
		try {
			for (int isupplier = 0; isupplier < SUPPLIER_NAMES.length-1; isupplier++) {
				outFile.println("(" + makeSupplierRecord(isupplier+1, "'" + SUPPLIER_NAMES[isupplier] + "'") + "),");
			}
			outFile.println("(" + makeSupplierRecord(SUPPLIER_NAMES.length, "'" + SUPPLIER_NAMES[SUPPLIER_NAMES.length-1] + "'") + ");");
		} finally {
			if (outFile != null) outFile.close();
		}
	}
	...
	public void writeSuppliesTable(String filename) throws Exception {
		...
		PrintWriter outFile = new PrintWriter (filename);
		String header = "INSERT INTO `Supplies` (`eid`, `sid`) VALUES";
		outFile.println(header);

		try {
			//generated first [getTotalSupplies()-1] records (each ends eith ",")
